...
class AbstractDetachedWindow:
    ...
    def __init__(self, master: tk.Tk, title: str, winType: Literal["tc", "tl"]):
        ...
        # Window option
        assert gClass.style is not None
        gClass.style.configure("TranslatedSub.TFrame", background=fJson.settingCache[f"ex_{winType}_bg"])
        ...
        if platform.system() == "Windows":
            self.click_through.set(int(fJson.settingCache[f"ex_{winType}_click_through"]))
        ...
        # init settings
        self.always_on_top.set(int(fJson.settingCache[f"ex_{winType}_always_on_top"]))
        self.toggle_always_on_top(fromKeyBind=False, onInit=True)

        self.no_title_bar.set(int(fJson.settingCache[f"ex_{winType}_no_title_bar"]))
        self.toggle_title_bar(fromKeyBind=False, onInit=True)

        self.no_tooltip.set(int(fJson.settingCache[f"ex_{winType}_no_tooltip"]))
    ...
    def toggle_tooltip(self, fromKeyBind=True, onInit=False):
        ...
        if not onInit:
            beep()
            fJson.savePartialSetting(f"ex_{self.winType}_no_tooltip", self.no_tooltip.get())
    ...
    def toggle_title_bar(self, fromKeyBind=True, onInit=False):
        ...
        if not onInit:
            beep()
            fJson.savePartialSetting(f"ex_{self.winType}_no_title_bar", self.no_title_bar.get())
    ...
    def toggle_click_through(self, fromKeyBind=True, onInit=False):
        ...
        if not onInit:
            beep()
            fJson.savePartialSetting(f"ex_{self.winType}_click_through", self.click_through.get())

        if self.click_through.get() == 1:
            self.root.wm_attributes("-transparentcolor", fJson.settingCache[f"ex_{self.winType}_bg"])
        else:
    ...
    def toggle_always_on_top(self, fromKeyBind=True, onInit=False):
        ...
        if not onInit:
            beep()
            fJson.savePartialSetting(f"ex_{self.winType}_always_on_top", self.always_on_top.get())
