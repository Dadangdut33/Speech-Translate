...
from speech_translate.utils.helper import up_first_case
...
class ModResultInputDialog:
    ...
    def __init__(self, master, title: str, mode: Union[Literal["Refinement", "Alignment"], str], with_lang=False):
        ...
        if mode == "Refinement":
            ...
            self.mod_file_chooser = (("JSON (Whisper Result)", "*.json"), )
        else:
            # *Alignment
            ...
            self.mod_file_chooser = (("JSON (Whisper Result)", "*.json"), ("Text (Plain text)", "*.txt"))
        ...
        self.mod_file_entry = ttk.Entry(self.f_2)
        self.mod_file_entry.pack(side="left", fill="x", expand=True, padx=(0, 5))
        self.mod_file_entry.bind("<Key>", lambda e: "break")

        self.btn_result_file = ttk.Button(self.f_2, text="Browse", command=self.browse_mod_file)
        self.btn_result_file.pack(side="left")
        tk_tooltips(
            [self.mod_file_entry, self.btn_result_file],
        ...
        if with_lang:
            ...
            self.select_cb = ComboboxWithKeyNav(self.f_3, values=WHISPER_LIST_UPPED, state="readonly")
    ...
    def browse_source_file(self):
        ...
        if self.audio_file is not None and self.mod_file is not None:
    ...
    def browse_mod_file(self):
        temp = filedialog.askopenfilename(
            ...
            filetypes=self.mod_file_chooser,
        )

        if len(temp) > 0:
            self.mod_file = temp
            self.mod_file_entry.delete(0, "end")
            self.mod_file_entry.insert(0, self.mod_file)
    ...
    def add_file_pair(self):
        if self.audio_file is None or self.mod_file is None:
            return

        self.root.destroy()

    def cancel(self):
        if self.audio_file is not None or self.mod_file is not None or self.lang_value is not None:
    ...
    def get_input(self):
        ...
        return self.audio_file, self.mod_file, self.lang_value
...
class AlignmentDialog(FileOperationDialog):
    ...
    def __init__(self, master, title: str, submit_func, theme: str, **kwargs):
        ...
        self.hint.pack(side="right", padx=5)
        tk_tooltip(
            self.hint, "This can be used to aligned/synced Audio with plain text or json result on word-level", wrap_len=300
        )
